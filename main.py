...
TRANSCRIBE_OUTPUT_FILE = BASE_DIR / "output" / "test3_transcribed_output.txt"
PUNCTUATED_OUTPUT_FILE = BASE_DIR / "output" / "test3_punctuated_output.txt"
SENTENCE_OUTUTPUT_FILE = BASE_DIR / "output" / "test3_sentences_output.txt"
...
# punctuated_text = restore_punctuation(transcribed_text)

# try:
#     with TRANSCRIBE_OUTPUT_FILE.open('r', encoding='utf-8') as f_in:
#         transcribed_text = f_in.read()
...
#         chunk_transcriptions = preprocess_text(punc_text, 'mar_Deva')

#         with SENTENCE_OUTUTPUT_FILE.open('w', encoding='utf-8') as f_out:
...
with SENTENCE_OUTUTPUT_FILE.open('r', encoding='utf-8') as f_in:
    chunk_transcriptions = [line.strip() for line in f_in if line.strip()] 
...
frequencies = wrf.get_frequencies(chunk_transcriptions)

results_list = []
for chunk_id, t in enumerate(chunk_transcriptions, start=1):
